...
type RouteDeps = {
  ...
  signAccessToken: (payload: { id: number; email: string; name?: string }) => string;
  signRefreshToken: (payload: { id: number; email: string }) => string;
...
export function createRoutes({ pool, refreshStore, signAccessToken, signRefreshToken, authenticateToken, isValidPassword, jwt, bcrypt }: RouteDeps) {
  ...
  router.post("/token", (req: Request<{}, {}, TokenBody>, res: Response) => {
    ...
    jwt.verify(token, process.env.REFRESH_TOKEN_SECRET || "CHANGE_ME_refresh_secret_!@#", (err, decoded) => {
      if (err) {
        return res.status(403).json({
          success: false,
          data: null,
